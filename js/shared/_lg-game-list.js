...
function GameListRenderer(_gc, _ui, _options) {
    ...
    this.loadAndRender = function (repeatLoad) {
        ...
        cs.sendRequest(GWT_GAMELIST, {
            ...
            $("#glLoadingImg").hide();
            if (result) {
                that.render(data.gameList, data.total);
                if (!repeatLoad) {
                    $("html, body").animate({
    ...
    this.bindHeaderAction = function (jId, columnSortBy, columnDefaultOrder) {
        $(jId).click(function () {
            ...
            count = GL_DEFAULT_COUNT;
            that.loadAndRender(true);
        });
    }

    this.render = function (gameList, total) {
        ui.hideHint();
...
//        }
        ...
        var tableContents = "<table id='gameListTable' class='standartTable' style='clear:both; border-collapse: collapse;' " +
            ...
            + "<th colspan='" + (that.options.showLabels ? 3 : 2) + "' width='" + width + "%'>"
            + "<span" + (sortBy == GL_SORT_BY_GAME_ID ? " class='actionText4 activeSortHeader'" : " class='actionText4'")
            + " id='glSortByGameId'><div class='glHeader'>Номер</div>"
            + (sortBy == GL_SORT_BY_GAME_ID ? ui.serverSortArrowsImg(order) + ui.getOrderHint(order) : " &nbsp;<img src='/img/icons/sort-both.png' alt=''/>")
            + "</span></th>"
            + "<th width='" + width + "%'><span" + (sortBy == GL_SORT_BY_GAME_RATING ? " class='actionText4 activeSortHeader'" : " class='actionText4'")
            + " id='glSortByGameRating'><div class='glHeader'>Рейтинг</div><div class='glHeaderDescr'>среднее время решивших игроков</div>"
            + (sortBy == GL_SORT_BY_GAME_RATING ? ui.serverSortArrowsImg(order) + ui.getOrderHint(order) : " &nbsp;<img src='/img/icons/sort-both.png' alt=''/>")
            + "</span></th>"
//            + (
//            cs.isSuperUser() ?
//                "<th width='" + width + "%'><span" + (sortBy == GL_SORT_BY_TOTAL_GAME_RATING ? " class='actionText4 activeSortHeader'" : " class='actionText4'") + " id='glSortByTotalGameRating'><div class='glHeader'>Полный рейтинг</div>"
//                    + (sortBy == GL_SORT_BY_TOTAL_GAME_RATING ? ui.serverSortArrowsImg(order) + ui.getOrderHint(order) : " &nbsp;<img src='/img/icons/sort-both.png' alt=''/>")
//                    + "</span></th>"
//                :
//                ""
//            )
            + "<th colspan='4' width='" + width + "%'><span" + (sortBy == GL_SORT_BY_PLAYER_RATING ? " class='actionText4 activeSortHeader'" : " class='actionText4'")
            + " id='glSortByPlayerRating'><div class='glHeader'>Ваше время /<br />место</div><div class='glHeaderDescr'>время решения</div>"
            + (sortBy == GL_SORT_BY_PLAYER_RATING ? ui.serverSortArrowsImg(order) + ui.getOrderHint(order) : " &nbsp;<img src='/img/icons/sort-both.png' alt=''/>")
            + "</span></th>"
            + "<th width='" + width + "%'><span" + (sortBy == GL_SORT_BY_TOTAL_WON ? " class='actionText4 activeSortHeader'" : " class='actionText4'")
            + " id='glSortByGameTotalWon'><div class='glHeader'>Выиграло</div>"
            + (sortBy == GL_SORT_BY_TOTAL_WON ? ui.serverSortArrowsImg(order) + ui.getOrderHint(order) : " &nbsp;<img src='/img/icons/sort-both.png' alt=''/>")
            + "</span></th>"
            + "<th width='" + width + "%'><span" + (sortBy == GL_SORT_BY_TOTAL_PLAYED ? " class='actionText4 activeSortHeader'" : " class='actionText4'")
            + " id='glSortByGameTotalPlayed'><div class='glHeader'>Играло</div>"
        ...
        var infinityParagraph = "<img src='/img/icons/infinity.png' alt='нет рейтинга'/>"
...
//        + "</p>"
            ...
            var tableData = "<td class='noRightBorder noLeftRightPadding'>" + (game.fav == 1 ? " &nbsp;<img src='/img/icons/fav-2.png' alt=''/>" : "") + "</td>"
                + "<td " + (that.options.showLabels ? "class='noRightBorder'" : "") + " style='text-align: right; white-space: nowrap; " + (that.options.showLabels ? "" : "padding-right: " + padding + "%;") + "'>"
                + gameIdParagraph
                + "</td>"
                + (that.options.showLabels ?
                "<td style='white-space: nowrap;'>" + labelParagraph + "</td>"
                : "")
                + "<td style='text-align: right; padding-right: " + padding + "%;'>" + gameRatingParagraph + "</td>"
//                + (
//                cs.isSuperUser()
//                    ?
//                    "<td style='text-align: right; padding-right: " + padding + "%;'>" + (totalGameRatingParagraph) + "</td>"
//                    :
//                    ""
//                )
                + playerWinTimeBlock
...
//    alert(t);
        ...
        if (count > 0 && count < total) {
            $("#gameListContents").append("<table border='1' style='margin-top: 10px;' width='100%' class='noBordersTable' id='glShowPanel'>"
                + "<tr>"
                + "<td width='50%'>"
                + "<p class='glShowMore' id='glShowMore'>Eщё " + Math.min(GL_DEFAULT_COUNT, total - count) + " " + that.options.moreLabel + "</p>"
                + "</td>"
                + "<td width='50%'>"
                + "<p class='glShowAll' id='glShowAll'>Показать все</p>"
                + "</td>"
                + "</tr>"
                + "<tr>"
                + "<td colspan='2' class='glPaginationStats' style='text-align: center !important;'>" + count + " из " + total + "</td>"
                + "</tr>"
                + "</table>");
        }
        ...
        for (var i = 0; i < gameList.length; i++) {
            ...
            $("#glPlay" + _gameId).click(function (_gameId) {
                return function () {
                    ...
                    return function () {
                        if (!$("#glGameDetailContent" + _gameId).is(":visible")) {
                            ...
                            $("#glGameDetailContent" + _gameId).empty().append("<div style=\"padding-top: 20px; padding-left:20px; height:35px;\">" +
                                "<span style=\"float: left; color:black;\">Загрузка&nbsp;" +
...
//        that.bindHeaderAction("#glSortByTotalGameRating", GL_SORT_BY_TOTAL_GAME_RATING, false);
        ...
        $("#glSortByGameId").mouseenter(function () {
            ui.showHint("#glSortByGameId", "Сортировать по<br />номеру расклада");
        });

        $("#glSortByGameRating").mouseenter(function () {
            ui.showHint("#glSortByGameRating", "Сортировать по среднему времени<br />среди всех решивших");
        });

        $("#glSortByPlayerRating").mouseenter(function () {
            ui.showHint("#glSortByPlayerRating", "Сортировать по<br />вашему времени решения");
        });

        $("#glSortByGameTotalWon").mouseenter(function () {
            ui.showHint("#glSortByGameTotalWon", "Сортировать по<br />количеству выигравших");
        });

        $("#glSortByGameTotalPlayed").mouseenter(function () {
            ui.showHint("#glSortByGameTotalPlayed", "Сортировать по<br />количеству игравших");
        });
        ...
        $("#glShowMore").click(function () {
            $("#glShowPanel").empty().append("<p class='giLoadingMore'>Загрузка&nbsp;<img style='margin-bottom:-10px;' src='/img/icons/loading.gif'></p>");
            count = count + GL_DEFAULT_COUNT;
            that.loadAndRender(true);
        });
